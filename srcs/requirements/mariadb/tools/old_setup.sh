#!/bin/sh

set -e

DB_PASSWORD=$(cat /run/secrets/db_password)
DB_ROOT_PASSWORD=$(cat /run/secrets/db_root_password)

init_database()
{
	rm -rf $DB_DATADIR/*
	echo "initializing db, datadir=" $DB_DATADIR
	mariadbd --user=mysql --datadir="$DB_DATADIR" --bootstrap << EOF

ALTER USER 'root'@'localhost' IDENTIFIED BY '${DB_ROOT_PASSWORD}';
CREATE DATABASE ${WORDPRESS_DB};
CREATE USER '${WP_DB_USER}'@'%' IDENTIFIED BY '${DB_PASSWORD}';
GRANT ALL PRIVILEGES ON ${WORDPRESS_DB}.* TO '${WP_DB_USER}'@'%';
FLUSH PRIVILEGES;
EOF
	unset DB_PASSWORD;
	unset DB_ROOT_PASSWORD;
	echo "END INIT"
}
	
mkdir -p "$DB_DATADIR"
chown -R mysql:mysql ${DB_DATADIR}
mariadb-install-db --user=mysql --datadir="$DB_DATADIR"

exec mysqld_safe
if [ ! -d "$DB_DATADIR/mysql" ]; then
	echo "INITIALIZING DATABASE!!!"
	rm -f /var/lib/mysql/mysql.sock
    	rm -f /tmp/mysql.sock

	exec mariadbd --user=mysql --datadir="$DB_DATADIR" --init-file=init.sql --bind-address=0.0.0.0 --port=3306 --socket=/dev/null

	echo "ITS FIRST TIME !!!"
else
	echo "ITS NOT FIRST TIME"
	exec mariadbd --user=mysql --datadir="$DB_DATADIR" --bind-address=0.0.0.0 --port=3306 --socket=/dev/null
fi

